...
static int avi_add_chunk(avi_t *AVI, const char *tag, unsigned char *data, int length)
{
   unsigned char c[8];
...
static int avi_add_index_entry(avi_t *AVI, const char *tag, long flags, long pos, long len)
{
   void *ptr;
...
static int avi_write_data(avi_t *AVI, char *data, long length, int audio)
{
   ...
   if(audio)
      n = avi_add_chunk(AVI,"01wb", (unsigned char *) data,length);
   else
      n = avi_add_chunk(AVI,"00db",(unsigned char*) data,length);
...
avi_t *AVI_open_input_file(const char *filename, int getIndex)
{
   ...
   if( strncasecmp((const char*) data  ,"RIFF",4) !=0 ||
       strncasecmp((const char*)(data+8),"AVI ",4) !=0 ) ERR_EXIT(AVI_ERR_NO_AVI)
   ...
   while(1)
   {
      ...
      if(strncasecmp( (const char*) data,"LIST",4) == 0)
      {
         if( read(AVI->fdes,data,4) != 4 ) ERR_EXIT(AVI_ERR_READ)
         n -= 4;
         if(strncasecmp((const char*)data,"hdrl",4) == 0)
         {
            hdrl_len = n;
         ...
         else if(strncasecmp((const char*)data,"movi",4) == 0)
         {
      ...
      else if(strncasecmp((const char*)data,"idx1",4) == 0)
      {
   ...
   for(i=0;i<hdrl_len;)
   {
      ...
      if(strncasecmp((const char*) (hdrl_data+i),"LIST",4)==0) { i+= 12; continue; }
      ...
      if(strncasecmp((const char*)(hdrl_data+i),"strh",4)==0)
      {
         i += 8;
         if(strncasecmp((const char*)(hdrl_data+i),"vids",4) == 0 && !vids_strh_seen)
         {
         ...
         else if (strncasecmp ((const char*)(hdrl_data+i),"auds",4) ==0 && ! auds_strh_seen)
         {
      ...
      else if(strncasecmp((const char*)(hdrl_data+i),"strf",4)==0)
      {
         i += 8;
         if(lasttag == 1)
         {
   ...
   if(AVI->idx)
   {
      ...
      for(i=0;i<AVI->n_idx;i++)
         if( strncasecmp((const char*)AVI->idx[i],AVI->video_tag,3)==0 ) break;
      ...
      while(1)
      {
         ...
         if( ( (data[2]=='d' || data[2]=='D') &&
          ...
          || ( (data[2]=='w' || data[2]=='W') &&
               (data[3]=='b' || data[3]=='B') ) )
         {
            avi_add_index_entry(AVI,(const char*)data,0,lseek(AVI->fdes,0,SEEK_CUR)-8,n);
         }
   ...
   for(i=0;i<AVI->n_idx;i++)
   {
      if(strncasecmp((const char*)AVI->idx[i],AVI->video_tag,3) == 0) nvi++;
      if(strncasecmp((const char*)AVI->idx[i],AVI->audio_tag,4) == 0) nai++;
   }
   ...
   for(i=0;i<AVI->n_idx;i++)
   {
      if(strncasecmp((const char*)AVI->idx[i],AVI->video_tag,3) == 0)
      {
         AVI->video_index[nvi].pos = str2ulong(AVI->idx[i]+ 8)+ioff;
         AVI->video_index[nvi].len = str2ulong(AVI->idx[i]+12);
         nvi++;
      }
      if(strncasecmp((const char*)AVI->idx[i],AVI->audio_tag,4) == 0)
      {
 ...
 */
   ...
   while(1)
   {
      ...
      if(strncasecmp((const char*)data,AVI->video_tag,3) == 0)
      {
         *len = n;
         AVI->video_pos++;
         if(n>max_vidbuf)
      ...
      else if(strncasecmp((const char*)data,AVI->audio_tag,4) == 0)
      {
         *len = n;
         if(n>max_audbuf)
         {
